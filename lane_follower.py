...
class HandCodedLaneFollower(object):
    ...
    def load_frame(self, frame):
        self.frameproc = FrameProc(frame)
    ...
    def follow_lane(self, frame):
        ...
        lane_lines, frame = self.frameproc.detect_lane()
    ...
    def steer(self, frame, lane_lines):
        ...
        new_steering_angle = self.frameproc.compute_steering_angle(lane_lines)
        self.curr_steering_angle = self.frameproc.stabilize_steering_angle(self.curr_steering_angle, new_steering_angle, len(lane_lines))
...
class FrameProc:
    ...
    def detect_lane(self):
        ...
        line_segment_image = display_lines(self.frame, line_segments)
    ...
    def detect_edges(self):
        ...
        show_image("hsv", hsv)
        lower_blue = np.array(_LOWER_BLUE)
        upper_blue = np.array(_UPPER_BLUE)
    ...
    def average_slope_intercept(self, line_segments):
        ...
        if len(left_fit) > 0:
            lane_lines.append(make_points(self.frame, left_fit_average))

        right_fit_average = np.average(right_fit, axis=0)
        if len(right_fit) > 0:
            lane_lines.append(make_points(self.frame, right_fit_average))

        logging.debug('lane lines: %s' % lane_lines)  # [[[316, 720, 484, 432]], [[1009, 720, 718, 432]]]

        return lane_lines

    def compute_steering_angle(self, lane_lines):
